...
export function copyPastingText() {
...
export function squigglePath(followPath, squiggleStep = 25, squiggleAmplitude = 20, offset = -1) {
...
export function remoteSaveState(state, callback) {
  //try {
...
export function getCategoriesFromNodes(nodes) {
...
export function stringDeterministicRandom(str) {
  return str.split('').reduce((num, char, i) => {
    return num + (char.charCodeAt(0) * (i+1)) % 256;
  }, 0);
}

export function getTagsFromNodes(nodes) {
      ...
      (typeof(nodes[nodeKey].metadata.tags) === 'string' ? nodes[nodeKey].metadata.tags.split(',') :  nodes[nodeKey].metadata.tags).map(tag => {
        if (tag.length > 0 && tag !== ' ' && !(tag in tagsObj)) {
          tagsObj[tag] = {
            ...
            dashArray: (80 + (stringDeterministicRandom(tag) % 70)) + ', 4, ' + (stringDeterministicRandom(tag) % 100) + ', 4'
          };
        }
      });
    }
...
export function getDomPosition(category, nodes) {
  const bounds = getBoundingRectangle(category, nodes);
  const c = (i, s) => {
    let r = bounds[i][0];
...
export function getBoundingRectangle(category, nodes) {
...
export function lineToSVGString(points, dir, simplex, style) {
  let st_1 = '',
    st_2 = '',
    a,
    b,
...
export function getClosestNodes(types, position, nodes, greedy) {
  let minDist = 9999999,
    closest = [],
    dist, i, type;
...
export function positionIsUsed(nodes, x, y) {
...
export function getClosestOpenPositionAndHitNode(position, nodes) {
  //FOLLOW A GRID SPIRAL PATTERN AWAY FROM position
  let stride = 16,
    step = 0,
    cx = position[0],
    cy = position[1] - 48;
  let r = positionIsUsed(nodes, cx, cy);
  if (!r) return [cx, cy, undefined];
  cx++;
  while (true) {
    //YOLO
    if (!positionIsUsed(nodes, cx, cy)) return [cx, cy, r];
...
export function getNodesWithinRect(nodes, coords) {
  const box = [
    [coords[0], coords[1]],
    [coords[2], coords[3]],
  ];
...
export function dragHighlight(start, selectCB, nodes) {
  let dragObj = {
    box: document.querySelector('#dragBox'),
    getNodesWithinRect: getNodesWithinRect,
    selectCB: selectCB,
    nodes: nodes,
    _sx: start[0],
    _sy: start[1],
...
export function addNodeAndConnections(
  newNodeType,
  newNodePosition,
  metadata,
  state
  ...
  */
  ...
  // Add or Move Item
  const position = getClosestOpenPositionAndHitNode(newNodePosition, state.nodes);
  position.pop();
  const update = {
      guid: guid(),
      position,
    ...
    categorys = getCategoriesFromNodes(state.nodes);
...
export function nodeTypesCompatible(nodeA, nodeB) {
...
export function getConnectorFromWants(fromNode, toNode) {
...
export function connectNewNodeAndNode(newNode, toNode, nodes, connections) {
  //Check that one node doesnt disallow the other
  if (!nodeTypesCompatible(newNode, nodes[toNode].type)) {
    return {
  ...
  let conn =
    getConnectorFromWants(newNode, nodes[toNode].type) ||
    getConnectorFromWants(nodes[toNode].type, newNode);
  if (conn) {
    conn.to = toNode;
    return conn;
  }
...
export function connectNodes(fromNode, toNode, nodes) {
  //Check that one node doesnt disallow the other
  if (!nodeTypesCompatible(nodes[toNode].type, nodes[fromNode].type)) {
    return {
      error:
  ...
  //Connection is possible, look in behavior metadata for recommended connection
  let conn = getConnectorFromWants(
    nodes[toNode].type,
    nodes[fromNode].type
  );
  if (conn) return conn;
  conn = getConnectorFromWants(
    nodes[fromNode].type,
    nodes[toNode].type
  );
  if (conn) return conn;
...
export function handleSingleAndDoubleClick(event, onSingle, onDouble) {
